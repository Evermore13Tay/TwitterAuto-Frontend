...
function ChangeProfilePage() {
    ...
    // 设备照片映射 (device_ip:u2_port -> {fileName, file})
            ...
            ws.current.onmessage = (event) => {
                ...
                try {
                    ...
                    if (data.type === 'status_update' && data.message) {
                        ...
                        // 任务完成消息
                        addStatusLog(`任务完成: ${data.message || '头像修改已完成'}`);
    ...
    const fetchDevices = async () => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/api/change-profile/devices`);
    ...
    // 处理照片选择
    const handlePhotoSelect = (device, event) => {
        ...
        addStatusLog(`已为设备 ${device.device_ip} 选择照片: ${file.name}`);
    };
    
    // 单个设备修改头像
    const handleSinglePhotoChange = async (device) => {
        const deviceKey = `${device.device_ip}:${device.u2_port}`;
        
        // 检查是否已选择照片
        if (!devicePhotos[deviceKey] || !devicePhotos[deviceKey].file) {
            addStatusLog(`错误: 设备 ${device.device_ip} 没有选择照片`);
            return;
        }
        
        setIsProcessing(true);
        setStatusLogs([]);
        addStatusLog(`开始修改设备 ${device.device_ip} 的头像...`);
        
        try {
            console.log("设备信息:", device); // 调试信息
            const formData = new FormData();
            formData.append('device_data', JSON.stringify(device));
            formData.append('photo', devicePhotos[deviceKey].file);
            
            const response = await fetch(`${API_BASE_URL}/api/change-profile/single`, {
                method: 'POST',
                body: formData,
            });
            ...
            if (data.success) {
                addStatusLog(`单个设备头像修改任务已创建 (ID: ${data.task_id})`);
    ...
    const handleStopTask = async () => {
        try {
            const response = await fetch(`${API_BASE_URL}/api/change-profile/stop`, {
                method: 'POST',
                headers: {
    ...
    return (
        <div className="space-y-8">
            <h1 className="text-3xl font-bold text-gray-800">修改头像</h1>
                        ...
                        <table className="min-w-full divide-y divide-gray-200 border border-gray-300">
                            <thead className="bg-gray-100">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" style={{width: '240px'}}>头像照片</th>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {devices.map((device, index) => {
                                    const deviceKey = `${device.device_ip}:${device.u2_port}`;
                                    const photoData = devicePhotos[deviceKey];
                                    ...
                                    return (
                                        <tr key={index} className={index % 2 === 0 ? '' : 'bg-gray-50'}>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                <div className="flex items-center space-x-2">
                                                    <span className="truncate max-w-[120px]">
                                                        {photoData ? photoData.fileName : '未选择'}
                                                    </span>
                                                    <label className="px-3 py-1 bg-gray-200 hover:bg-gray-300 text-gray-800 text-sm font-semibold rounded cursor-pointer" style={{width: '80px', textAlign: 'center'}}>
                                                        选择照片
                                                        <input
                                                            type="file"
                                                            accept="image/*"
                                                            className="hidden"
                                                            onChange={(e) => handlePhotoSelect(device, e)}
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                <button
                                                    onClick={() => handleSinglePhotoChange(device)}
                                                    disabled={!photoData || isProcessing}
