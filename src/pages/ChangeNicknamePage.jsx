...
function ChangeNicknamePage() {
    ...
    // 设备昵称映射 (device_ip:u2_port -> nickname)
            ...
            ws.current.onmessage = (event) => {
                ...
                try {
                    ...
                    if (data.type === 'status_update' && data.message) {
                        ...
                        // 任务完成消息
                        addStatusLog(`任务完成: ${data.message || '昵称修改已完成'}`);
    ...
    const fetchDevices = async () => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/api/change-nickname/devices`);
    ...
    // 处理昵称输入
    const handleNicknameChange = (device, event) => {
        const newNickname = event.target.value;
        const deviceKey = `${device.device_ip}:${device.u2_port}`;
        
        setDeviceNicknames(prev => ({
            ...prev,
            [deviceKey]: newNickname
        }));
    };
    
    // 单个设备修改昵称
    const handleSingleNicknameChange = async (device) => {
        const deviceKey = `${device.device_ip}:${device.u2_port}`;
        
        // 检查是否已输入昵称
        const nickname = deviceNicknames[deviceKey];
        if (!nickname || !nickname.trim()) {
            addStatusLog(`错误: 设备 ${device.device_ip} 没有输入昵称`);
            return;
        }
        
        setIsProcessing(true);
        setStatusLogs([]);
        addStatusLog(`开始修改设备 ${device.device_ip} 的昵称...`);
        ...
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/api/change-nickname/single`, {
                method: 'POST',
                headers: {
            ...
            if (data.success) {
                addStatusLog(`单个设备昵称修改任务已创建 (ID: ${data.task_id})`);
    ...
    const handleStopTask = async () => {
        try {
            const response = await fetch(`${API_BASE_URL}/api/change-nickname/stop`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    return (
        <div className="space-y-8">
            <h1 className="text-3xl font-bold text-gray-800">修改昵称</h1>
                        ...
                        <table className="min-w-full divide-y divide-gray-200 border border-gray-300">
                            <thead className="bg-gray-100">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">新昵称</th>
                                        ...
                                        <tr key={index} className={index % 2 === 0 ? '' : 'bg-gray-50'}>
                                            ...
                                            <td className="px-6 py-4 text-sm text-gray-500">
                                                <input
                                                    ...
                                                    value={nickname}
                                                    onChange={(e) => handleNicknameChange(device, e)}
                                                    placeholder="请输入新昵称"
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                <button
                                                    onClick={() => handleSingleNicknameChange(device)}
